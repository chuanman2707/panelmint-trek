import { BedDouble, Car, ChevronDown, ChevronUp, Footprints, Pencil, Route, StickyNote, Ticket, X } from 'lucide-react'
...
import { transportSubtitle, type TransportEntry } from './planTimelineModel'
...
/**
 * The row types of the mobile day timeline (place / transport / travel-time
 * connector / note), each in its go and edit variant. Edit rows swap the left
 * avatar for right-hand action circles.
 *
...
export function HotelConnRow({ seg, name, placement }: {
  ...
  // Same line as the connectors between two stops, so a bookend leg set to
  // walking reads the same as the rest of the day (#1281).
  const travel = (
