/**
 * Port of server/src/nest/weather/weather.impl.ts plus the response-size caps
 * from server/src/utils/cappedFetch.ts.
 *
 * Browser-compatible throughout: fetch is the platform global, Buffer became
 * Uint8Array, and the NodeJS.Timeout is a plain interval handle. The cache and
 * in-flight dedup are module state exactly as on the server; the cleanup sweep
 * still starts via startCacheCleanup() (a client's lifecycle hook calls it —
 * there is no Nest onModuleInit here).
 *
 * Server behaviours preserved:
 *  - estimateCondition thresholds (precip > 5 → rain/snow at 0°C, > 1 →
 *    drizzle/snow, > 0.3 → clouds, else clear above 15°C).
 *  - Cache keys round lat/lng to two decimals and carry date+language.
 *  - TTLs: forecast 1h, current 15min, climate 24h; cache capped at 1000
 *    entries pruned to 500.
 *  - Forecast window −1..+16 days, recent-past past_days window (−1..−5 without
 *    a usable time), ERA5 archive for the actual past, last-year's-climate for
 *    far-future dates; a request with a usable HH:MM stays on the archive for
 *    the hourly series.
 *  - Coordinate validation rejects empty and out-of-range values (400); a
 *    capped/misbehaving provider answer is a 502-shaped ApiError.
 *  - Response bodies are size-capped at 1 MB whether the length is declared or
 *    streamed.
 */

// ── cappedFetch.ts, verbatim (Buffer → Uint8Array) ───────────────────────────
...
/**
 * readCappedText plus the parse. Answers `undefined` when the body is over the
 * cap, was cut off at it, or does not parse.
 */
...
/**
 * Read a body, stopping once maxBytes have arrived. Responses without a stream
 * fall back to arrayBuffer(), which is post-checked instead.
 */
export async function readCapped(
  ...
  if (!reader) {
    const raw = res.arrayBuffer
      ...
      : new TextEncoder().encode(res.text ? await res.text() : '');
...
/** readCapped for text bodies, decoded UTF-8. */
export async function readCappedText(
  res: CappedResponse,
  maxBytes: number
): Promise<{ text: string; truncated: boolean }> {
  if (!res.body?.getReader && res.text) {
    const raw = new TextEncoder().encode(await res.text());
    return raw.length > maxBytes
      ? { text: new TextDecoder('utf-8').decode(raw.subarray(0, maxBytes)), truncated: true }
      : { text: new TextDecoder('utf-8').decode(raw), truncated: false };
  }
  const { bytes, truncated } = await readCapped(res, maxBytes);
  return { text: new TextDecoder('utf-8').decode(bytes), truncated };
}

// ── weather.impl.ts, verbatim ────────────────────────────────────────────────

// Open-Meteo sits on the request path as a third party: without a deadline a hung
// connection holds the caller until the socket gives up on its own.
const WEATHER_TIMEOUT_MS = 8000;

/**
 * Which slot of Open-Meteo's hourly series a HH:MM belongs to. The archive
 * answers with one entry per hour of the requested day in the local timezone, so
 * the hour is the index. Anything unparseable means "no time given".
 */
 ...
 * Open-Meteo URL, so the boundary check lives here.
 */
...
/** Start the cache sweep. A client's lifecycle hook calls this — there is no
 *  Nest onModuleInit here. Idempotent. */
export function startCacheCleanup(): void {
  if (cleanupTimer) return;
  cleanupTimer = setInterval(pruneCache, CACHE_CLEANUP_INTERVAL);
  // Never hold the process open for a cache sweep (Node timers only; a browser
  // interval has no unref and the guard is a no-op there).
...
/** Test hook: drop all cached answers and in-flight records. The server tests
 *  reset module state per case; the browser port offers the same door. */
 ...
 * Returns null when the requested date is not in the answered series.
 */
...
async function _getWeatherImpl(
  ...
  if (date) {
    ...
    if (diffDays >= -1 && diffDays <= 16) {
      ...
      // idx === -1 with diffDays <= 0: a location-local yesterday that the
      // forward window no longer carries — ask past_days before the archive.
      if (diffDays <= 0) {
    ...
    // stays on the archive — only it has the hourly series.
