...
function contentRefusal(err: FileContentError): string {
  if (err.reason === 'too-large') {
    return `File is too large to read here (over ${CONTENT_MAX_MB} MB). Ask the user to open it in PanelMint instead.`;
  }
