...
describe('buildWebhookBody', () => {
  ...
  it('Discord embed footer defaults to PanelMint when no trip name', () => {
    ...
    expect(body.embeds[0].footer.text).toBe('PanelMint');
  });
  ...
  it('generic URL produces plain JSON with original fields plus timestamp and source', () => {
    ...
    expect(body).toHaveProperty('source', 'PanelMint');
  });
});
...
describe('buildEmailHtml', () => {
  ...
  it('uses English i18n strings for lang=en', () => {
    ...
    expect(html).toContain('notifications enabled in PanelMint');
  });

  it('uses German i18n strings for lang=de', () => {
    const html = buildEmailHtml('Subject', 'Body', 'de');
    expect(html).toContain('PanelMint aktiviert');
  });
  ...
  it('falls back to English i18n for unknown language', () => {
    ...
    expect(unknown).toContain('notifications enabled in PanelMint');
  });
});
