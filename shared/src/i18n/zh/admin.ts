...
const admin: TranslationStrings = {
  ...
  'admin.apiKeysHint': '地点数据从哪里来。PanelMint 索引不需要密钥，下面两个提供方是可选的。',
  'admin.trekApi.badgeDefault': '推荐的默认来源',
  'admin.googleCaveat.badge': '不推荐',
  'admin.googleCaveat.body':
    'PanelMint 是开源软件，在这件事上我们并不中立。在这个规模上，评分和普通店铺的照片只有 Google 有，这就是垄断。这个输入框存在是因为没有替代品，而不是因为我们推荐。填了它，每次查询都会发往 Google。',
  'admin.trekApi.tagline':
    'PanelMint 自己的地点索引。不用 Google 密钥，没有配额，也没有人统计你搜了什么。',
  ...
  'admin.placesProvider.subtitle':
    'PanelMint 自有索引和 OpenStreetMap 会回应每一次搜索。这里选择的是它们一无所获时还问谁：自动在有密钥时优先 Google，其次 Amap。',
  'admin.placesProvider.auto': '自动',
  'admin.placesProvider.google': 'Google Places',
  'admin.placesProvider.amap': '高德地图',
  'admin.placesProvider.openstreetmap': 'OpenStreetMap',
  'admin.placesProvider.missingKey': '所选服务商未配置 API 密钥，因此地点搜索仅由 PanelMint 索引和 OpenStreetMap 回应。',
  ...
  'admin.plugins.requiresTrek': '需要 PanelMint {version}+',
  ...
  'admin.plugins.perm.hook:search-provider': '从自有索引回答地点搜索，与 PanelMint 自身的结果并列显示',
  ...
  'admin.plugins.perm.geolocation:read': '在其某个视图打开时请求你的实时位置（由 PanelMint 使用本站点已有的位置权限读取，不会单独为插件询问）',
  ...
  'admin.plugins.reviewedMeaning':
    '“已审核”表示 PanelMint 维护者已针对该插件的每个版本进行了恶意软件扫描 —— 而非检查其质量或是否可用。这并不保证插件无害。',
  ...
  'admin.plugins.security.isolationBody':
    '插件作为其独立的受限进程运行，只能读取自己的文件。它无法读取你的数据库、登录密钥或加密密钥，无法启动其他程序，也无法在任何位置写入文件。它的界面运行在一个封闭的浏览器框架中，既读不到你的会话 Cookie，也无法触及周围的 PanelMint 页面。',
  'admin.plugins.security.permsTitle': '权限的含义',
  'admin.plugins.security.permsBody':
    '安装前显示的权限是 PanelMint 在插件运行期间强制执行的硬性上限 —— 凡是不在清单上的事，插件根本无法做到。但它们只告诉你插件能做什么，而非它实际做了什么。一个获准读取你的行程并连接某服务器的插件，就能把你的行程发送到该服务器，因此请仔细阅读权限和出站主机，而不只是看描述。',
  'admin.plugins.security.limitsTitle': '我们无法承诺的事',
  'admin.plugins.security.limitsBody':
    '这种隔离是强有力的软件边界，但并非绝对。插件所拥有的权限完全就是你批准的那些，因此在这些权限范围内，它的行为可能与其描述不符，也可能把它合法持有的数据发送到它所声明的主机。PanelMint 不会读取或评判插件代码实际做了什么。',
  ...
  'admin.plugins.security.reviewedBody':
    '已审核的插件，其每个版本都经过 PanelMint 维护者的人工恶意软件扫描 —— 检查的是有无恶意代码，而非它是否运行良好。这并不保证插件无害。',
  'admin.plugins.security.signedTitle': '“已签名”的含义',
  'admin.plugins.security.signedBody':
    'PanelMint 在每次安装时校验的校验和，证明这些文件正是插件库所担保的那些文件。签名证明的则是另一回事：它们出自作者本人，并以只有作者持有的密钥签署。已签名的插件两者兼备。未签名并不意味着不安全 —— 它只是少了一重保证，而目前插件库中的大多数插件都未签名。',
  ...
  'admin.plugins.sig.missingBody':
    '你安装此插件时它是有签名的，但新版本未附带任何签名。PanelMint 不会悄然接受这种降级。此项无法忽略。',
  ...
  'admin.plugins.sig.confirmOutOfBand':
    'PanelMint 无法区分正当的密钥轮换与一次接管 —— 从这里看，两者别无二致。在接受之前，请通过你原本就信任的渠道向作者核实这把新密钥。核实并接受后，插件将被更新，新密钥也会被记住。',
  ...
  'admin.plugins.security.trustBody':
    '安装插件就如同安装任何第三方应用：只添加来自你信任的作者的代码，如有疑虑，请先自行检查。PanelMint 对第三方插件不承担任何责任。',
  ...
  'admin.plugins.dep.trekIncompatible': '需要 PanelMint {range}——此服务器运行的是 {host}',
  'admin.plugins.dep.trekUnknown': '未声明支持的 PanelMint 版本',
  ...
  'admin.plugins.rangeBypass.title': '超出支持的 PanelMint 版本',
  'admin.plugins.rangeBypass.noticeTitle': '已安装在支持的 PanelMint 版本之外',
  ...
  'admin.plugins.dep.trekBypassed': '超出其 PanelMint 范围（{range}）— 版本检查已关闭',
  'admin.plugins.dep.trekBypassedUnknown': '未声明 PanelMint 范围 — 版本检查已关闭',
  ...
  'admin.addons.subtitle': '启用或禁用功能以自定义你的 PanelMint 体验。',
  ...
  'admin.weather.description': 'PanelMint 使用 Open-Meteo 作为天气数据源。Open-Meteo 是免费的开源天气服务——无需 API 密钥。',
  ...
  'admin.update.text': 'PanelMint {version} 已发布。你当前使用的是 {current}。',
  'admin.update.button': '在 GitHub 查看',
  'admin.update.install': '安装更新',
  'admin.update.confirmTitle': '确定安装更新？',
  'admin.update.confirmText': 'PanelMint 将从 {current} 更新到 {version}。服务器将自动重启。',
  ...
  'admin.update.dockerText': '你的 PanelMint 实例运行在 Docker 中。要更新到 {version}，请在服务器上执行以下命令：',
  'admin.update.nonDockerText':
    '此 PanelMint 实例未运行在 Docker 中。要更新到 {version}，请重新执行你当初使用的安装或更新方式——例如，在 Proxmox Community Scripts 上，从 LXC 控制台运行更新：',
