...
const admin: TranslationStrings = {
  ...
  'admin.apiKeysHint': '地點資料從哪裡來。PanelMint 索引不需要金鑰，下面兩個提供者是選用的。',
  'admin.trekApi.badgeDefault': '推薦的預設來源',
  'admin.googleCaveat.badge': '不建議',
  'admin.googleCaveat.body':
    'PanelMint 是開源軟體，在這件事上我們並不中立。在這個規模上，評分和一般店家的照片只有 Google 有，這就是壟斷。這個欄位存在是因為沒有替代品，而不是因為我們推薦。填了它，每次查詢都會送往 Google。',
  'admin.trekApi.tagline':
    'PanelMint 自己的地點索引。不用 Google 金鑰，沒有配額，也沒有人統計你搜了什麼。',
  ...
  'admin.placesProvider.subtitle':
    'PanelMint 自有索引和 OpenStreetMap 會回應每一次搜尋。這裡選擇的是它們一無所獲時還問誰：自動在有金鑰時優先 Google，其次 Amap。',
  'admin.placesProvider.auto': '自動',
  'admin.placesProvider.google': 'Google Places',
  'admin.placesProvider.amap': '高德地圖',
  'admin.placesProvider.openstreetmap': 'OpenStreetMap',
  'admin.placesProvider.missingKey': '所選服務商未設定 API 金鑰，因此地點搜尋僅由 PanelMint 索引和 OpenStreetMap 回應。',
  ...
  'admin.plugins.perm.hook:search-provider': '從自有索引回答地點搜尋，與 PanelMint 自身的結果並列顯示',
  ...
  'admin.plugins.perm.geolocation:read': '在其任一檢視開啟時查詢你的即時位置（由 PanelMint 使用本站台既有的位置權限讀取，不會另外為外掛詢問）',
  ...
  'admin.plugins.reviewedMeaning':
    '「已審核」代表 PanelMint 維護者曾針對每個版本掃描此外掛是否含有惡意程式碼——並非檢查品質或功能是否正常。這不保證外掛無害。',
  ...
  'admin.plugins.security.isolationBody':
    '外掛以自身受限的獨立程序執行，只能讀取自己的檔案。它無法讀取你的資料庫、登入密鑰或加密金鑰，無法啟動其他程式，也無法在任何位置寫入檔案。它的介面在一個封閉的瀏覽器框架中執行，無法讀取你的工作階段 Cookie，也無法觸及其周圍的 PanelMint 頁面。',
  'admin.plugins.security.permsTitle': '這些權限的含義',
  'admin.plugins.security.permsBody':
    '安裝前顯示的權限是 PanelMint 在外掛執行期間強制執行的硬性上限——它絕對無法做出清單以外的任何動作。但這些權限告訴你外掛「能」做什麼，而非它「實際」做什麼。一個獲准讀取你行程並連線某伺服器的外掛，就能將你的行程傳送到該伺服器，所以請閱讀權限與對外主機，而不只是看說明。',
  'admin.plugins.security.limitsTitle': '我們無法保證的事',
  'admin.plugins.security.limitsBody':
    '這種隔離是一道強而有力的軟體邊界，但並非絕對。外掛僅以你核准的權限行事，因此在這些權限範圍內，它的行為可能與其說明不符，也可能將它合法持有的資料傳送到它所宣告的主機。PanelMint 不會讀取或評判外掛的程式碼實際做了什麼。',
  ...
  'admin.plugins.security.reviewedBody':
    '已審核的外掛，代表 PanelMint 維護者曾針對每個版本手動掃描惡意程式碼——檢查是否含有有害程式碼，而非它是否運作良好。這並不保證外掛無害。',
  'admin.plugins.security.signedTitle': '「已簽署」的含義',
  'admin.plugins.security.signedBody':
    'PanelMint 在每次安裝時驗證的總和檢查碼，證明這些檔案正是外掛庫所擔保的檔案。簽章證明的則是另一回事：它們出自作者之手，並以只有作者持有的金鑰簽署。已簽署的外掛兩者兼備。未簽署並不代表不安全——它只是少了一層保證，而目前外掛庫中的多數外掛都尚未簽署。',
  ...
  'admin.plugins.sig.missingBody':
    '你安裝此外掛時它是有簽章的，但新版本並未附上任何簽章。PanelMint 不會默默接受這種倒退。此項無法略過。',
  ...
  'admin.plugins.sig.confirmOutOfBand':
    'PanelMint 無法分辨正當的金鑰輪替與一次接管——從這裡看，兩者毫無二致。在接受之前，請透過你原本就信任的管道向作者確認這把新金鑰。一旦你接受，外掛就會更新，新金鑰也會被記住。',
  ...
  'admin.plugins.security.trustBody':
    '安裝外掛就像安裝任何第三方應用程式：只加入你信任的作者所提供的程式碼，如有疑慮，請先自行檢查。PanelMint 對第三方外掛概不承擔任何責任。',
  ...
  'admin.plugins.dep.trekIncompatible': '需要 PanelMint {range}——此伺服器執行的是 {host}',
  'admin.plugins.dep.trekUnknown': '未宣告支援的 PanelMint 版本',
  ...
  'admin.plugins.rangeBypass.title': '超出支援的 PanelMint 版本',
  'admin.plugins.rangeBypass.noticeTitle': '已安裝在支援的 PanelMint 版本之外',
  ...
  'admin.plugins.dep.trekBypassed': '超出其 PanelMint 範圍（{range}）— 版本檢查已關閉',
  'admin.plugins.dep.trekBypassedUnknown': '未宣告 PanelMint 範圍 — 版本檢查已關閉',
  ...
  'admin.addons.subtitle': '啟用或停用功能以自定義你的 PanelMint 體驗。',
  ...
  'admin.weather.description':
    'PanelMint 使用 Open-Meteo 作為天氣資料來源。Open-Meteo 是免費的開源天氣服務——無需 API 金鑰。',
  ...
  'admin.update.text': 'PanelMint {version} 已釋出。你當前使用的是 {current}。',
  'admin.update.button': '在 GitHub 檢視',
  'admin.update.install': '安裝更新',
  'admin.update.confirmTitle': '確定安裝更新？',
  'admin.update.confirmText': 'PanelMint 將從 {current} 更新到 {version}。伺服器將自動重啟。',
  ...
  'admin.update.dockerText': '你的 PanelMint 例項執行在 Docker 中。要更新到 {version}，請在伺服器上執行以下命令：',
  'admin.update.nonDockerText':
    '此 PanelMint 例項並非執行在 Docker 中。要更新到 {version}，請重新執行你當初使用的安裝或更新方式——例如在 Proxmox Community Scripts 上，請從 LXC 主控臺執行更新：',
