...
const cs: NotificationLocale = {
  email: {
    footer: 'Toto jsi obdržel/a, protože máš povoleny upozornění v PanelMint.',
    manage: 'Spravovat předvolby v nastavení',
    madeWith: 'Made with',
    openTrek: 'Otevřít PanelMint',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} vás pozval ke spojení dovolenkových plánů. Otevřete PanelMint pro přijetí nebo odmítnutí.`,
    }),
    vacay_share: (p) => ({
      title: 'Kalendář Vacay sdílen',
      body: `${p.actor} s vámi sdílel svůj kalendář dovolených. Otevřete PanelMint pro zobrazení.`,
    }),
    collection_invite: (p) => ({
      title: 'Pozvánka do sbírky',
      body: `${p.actor} vás pozval ke sdílení sbírky. Otevřete PanelMint pro přijetí nebo odmítnutí.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nová verze PanelMint dostupná',
      body: `PanelMint ${p.version} je nyní dostupný. Navštivte administrátorský panel pro aktualizaci.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Ahoj',
    body: 'Obdrželi jsme žádost o obnovení hesla k tvému účtu PanelMint. Klikni na tlačítko níže a nastav nové heslo.',
