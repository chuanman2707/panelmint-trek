...
const pl: NotificationLocale = {
  email: {
    footer: 'Otrzymałeś/aś tę wiadomość, ponieważ masz włączone powiadomienia w PanelMint.',
    manage: 'Zarządzaj preferencjami w ustawieniach',
    madeWith: 'Made with',
    openTrek: 'Otwórz PanelMint',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} zaprosił Cię do połączenia planów urlopowych. Otwórz PanelMint, aby zaakceptować lub odrzucić.`,
    }),
    vacay_share: (p) => ({
      title: 'Kalendarz Vacay udostępniony',
      body: `${p.actor} udostępnił Ci swój kalendarz urlopów. Otwórz PanelMint, aby go zobaczyć.`,
    }),
    collection_invite: (p) => ({
      title: 'Zaproszenie do kolekcji',
      body: `${p.actor} zaprosił Cię do udostępnienia kolekcji. Otwórz PanelMint, aby zaakceptować lub odrzucić.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nowa wersja PanelMint dostępna',
      body: `PanelMint ${p.version} jest teraz dostępny. Odwiedź panel administracyjny, aby zaktualizować.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Cześć',
    body: 'Otrzymaliśmy prośbę o zresetowanie hasła do Twojego konta PanelMint. Kliknij przycisk poniżej, aby ustawić nowe hasło.',
