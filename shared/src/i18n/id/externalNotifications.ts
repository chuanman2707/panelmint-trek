...
const id: NotificationLocale = {
  email: {
    footer: 'Anda menerima ini karena Anda telah mengaktifkan notifikasi di PanelMint.',
    manage: 'Kelola preferensi di Pengaturan',
    madeWith: 'Dibuat dengan',
    openTrek: 'Buka PanelMint',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} mengundang Anda untuk menggabungkan rencana liburan. Buka PanelMint untuk menerima atau menolak.`,
    }),
    vacay_share: (p) => ({
      title: 'Kalender Vacay Dibagikan',
      body: `${p.actor} membagikan kalender cutinya dengan Anda. Buka PanelMint untuk melihatnya.`,
    }),
    collection_invite: (p) => ({
      title: 'Undangan koleksi',
      body: `${p.actor} mengundang Anda untuk berbagi koleksi. Buka PanelMint untuk menerima atau menolak.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Versi PanelMint baru tersedia',
      body: `PanelMint ${p.version} sekarang tersedia. Kunjungi panel admin untuk memperbarui.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Halo',
    body: 'Kami menerima permintaan untuk menyetel ulang kata sandi akun PanelMint Anda. Klik tombol di bawah untuk menetapkan kata sandi baru.',
