...
const admin: TranslationStrings = {
  ...
  'admin.apiKeys': 'API 키',
  'admin.apiKeysHint': '장소 데이터의 출처. PanelMint 색인에는 키가 필요 없으며, 아래 두 제공자는 선택 사항입니다.',
  'admin.trekApi.badgeDefault': '권장 기본 소스',
  'admin.googleCaveat.badge': '권장하지 않음',
  'admin.googleCaveat.body':
    'PanelMint은 오픈 소스이며 이 점에서 중립이 아닙니다. 이 규모에서 평점과 일반 상점 사진은 Google에만 있고, 그것이 독점입니다. 이 입력란은 대안이 없어서 있는 것이지 권해서가 아닙니다. 쓰면 모든 조회가 Google로 갑니다.',
  'admin.trekApi.tagline':
    'PanelMint 자체 장소 색인. Google 키 없이, 할당량 없이, 누구도 검색 횟수를 세지 않는 채로 찾습니다.',
  ...
  'admin.placesProvider.subtitle':
    'PanelMint 자체 색인과 OpenStreetMap이 모든 검색에 답합니다. 여기서는 둘이 아무것도 찾지 못했을 때 누구에게 더 물을지 고릅니다. 자동은 키가 있으면 Google을, 그다음 Amap을 씁니다.',
  'admin.placesProvider.auto': '자동',
  'admin.placesProvider.google': 'Google Places',
  'admin.placesProvider.amap': 'Amap(高德地图)',
  'admin.placesProvider.openstreetmap': 'OpenStreetMap',
  'admin.placesProvider.missingKey': '선택한 공급자에 API 키가 설정되어 있지 않아 장소 검색은 PanelMint 색인과 OpenStreetMap만 응답합니다.',
  ...
  'admin.plugins.requiresTrek': 'PanelMint {version} 이상 필요',
  ...
  'admin.plugins.perm.hook:search-provider': '자체 색인에서 장소 검색에 응답 (PanelMint 자체 결과와 함께 표시)',
  ...
  'admin.plugins.perm.geolocation:read':
    '해당 플러그인 화면이 열려 있는 동안 실시간 위치 요청 (PanelMint이 이 사이트에 부여된 위치 권한으로 읽으며, 플러그인용 권한을 따로 묻지 않음)',
  ...
  'admin.plugins.reviewedMeaning':
    '"검토됨"은 PanelMint 관리자가 각 버전마다 이 플러그인에 악성코드가 있는지 검사했다는 의미입니다 — 품질이나 정상 작동 여부를 검사한 것이 아닙니다. 플러그인이 무해하다는 보장은 아닙니다.',
  ...
  'admin.plugins.security.isolationBody':
    '플러그인은 자체 파일만 읽을 수 있는 잠긴 별도 프로세스로 실행됩니다. 데이터베이스, 로그인 비밀 값, 암호화 키를 읽을 수 없고, 다른 프로그램을 실행할 수 없으며, 어디에도 파일을 쓸 수 없습니다. 플러그인의 인터페이스는 세션 쿠키를 읽거나 주변 PanelMint 페이지를 건드릴 수 없는 봉인된 브라우저 프레임에서 실행됩니다.',
  'admin.plugins.security.permsTitle': '권한이 의미하는 것',
  'admin.plugins.security.permsBody':
    '설치 전에 표시되는 권한은 플러그인이 실행되는 동안 PanelMint이 강제하는 엄격한 한계입니다 — 목록에 없는 것은 말 그대로 아무것도 할 수 없습니다. 하지만 이는 플러그인이 무엇을 할 수 있는지를 알려줄 뿐, 실제로 무엇을 하는지는 알려주지 않습니다. 여행을 읽고 서버에 연결할 수 있는 플러그인은 그 서버로 여행 데이터를 전송할 수 있으므로, 설명뿐만 아니라 권한과 아웃바운드 호스트도 확인하세요.',
  'admin.plugins.security.limitsTitle': '보장할 수 없는 것',
  'admin.plugins.security.limitsBody':
    '이 격리는 강력한 소프트웨어 경계이지만 절대적인 것은 아닙니다. 플러그인은 정확히 승인한 권한만큼 동작하므로, 그 권한 범위 내에서는 설명과 다르게 동작할 수 있으며, 정당하게 보유한 데이터를 선언한 호스트로 전송할 수 있습니다. PanelMint은 플러그인의 코드가 실제로 무엇을 하는지 읽거나 판단하지 않습니다.',
  ...
  'admin.plugins.security.reviewedBody':
    '검토된 플러그인은 각 버전마다 PanelMint 관리자가 악성코드를 수동으로 검사한 것입니다 — 정상 작동 여부가 아니라 악의적인 코드가 있는지 확인한 것입니다. 플러그인이 무해하다는 약속은 아닙니다.',
  'admin.plugins.security.signedTitle': '"서명됨"의 의미',
  'admin.plugins.security.signedBody':
    'PanelMint이 설치할 때마다 검증하는 체크섬은 파일이 레지스트리가 보증하는 것과 정확히 일치함을 증명합니다. 서명은 다른 것을 증명합니다. 그 파일이 작성자에게서 왔으며, 작성자만 가진 키로 서명되었다는 사실입니다. 서명된 플러그인은 두 가지를 모두 갖추고 있습니다. 서명되지 않았다고 해서 안전하지 않다는 뜻은 아닙니다 — 보장이 하나 적을 뿐이며, 현재 레지스트리의 플러그인 대부분은 서명되어 있지 않습니다.',
  ...
  'admin.plugins.sig.missingBody':
    '설치할 때 이 플러그인에는 서명이 있었지만, 새 버전에는 서명이 없습니다. PanelMint은 이런 후퇴를 조용히 받아들이지 않습니다. 이 경우는 무시하고 진행할 수 없습니다.',
  ...
  'admin.plugins.sig.confirmOutOfBand':
    'PanelMint은 정당한 키 교체와 탈취를 구분할 수 없습니다 — 여기서는 둘이 똑같아 보입니다. 받아들이기 전에 이미 신뢰하는 경로를 통해 작성자에게 새 키를 확인하세요. 받아들이면 플러그인이 업데이트되고 새 키가 기억됩니다.',
  ...
  'admin.plugins.security.trustBody':
    '플러그인을 설치하는 것은 다른 서드파티 앱을 설치하는 것과 같습니다: 신뢰할 수 있는 작성자의 코드만 추가하고, 의심스러우면 먼저 직접 확인하세요. PanelMint은 서드파티 플러그인에 대해 어떠한 책임도 지지 않습니다.',
  ...
  'admin.plugins.dep.trekIncompatible': 'PanelMint {range}이(가) 필요합니다 — 이 서버는 {host}입니다',
  'admin.plugins.dep.trekUnknown': '지원하는 PanelMint 버전을 명시하지 않았습니다',
  ...
  'admin.plugins.rangeBypass.title': '지원되는 PanelMint 버전 범위 밖',
  'admin.plugins.rangeBypass.noticeTitle': '지원되는 PanelMint 버전 범위 밖에 설치됨',
  ...
  'admin.plugins.dep.trekBypassed': 'PanelMint 범위 밖 ({range}) — 버전 검사 꺼짐',
  'admin.plugins.dep.trekBypassedUnknown': 'PanelMint 범위 선언 없음 — 버전 검사 꺼짐',
  ...
  'admin.addons.subtitle': '기능을 활성화 또는 비활성화하여 PanelMint 경험을 맞춤 설정하세요.',
  ...
  'admin.weather.description':
    'PanelMint은 날씨 데이터 소스로 Open-Meteo를 사용합니다. Open-Meteo는 무료 오픈 소스 날씨 서비스로 API 키가 필요 없습니다.',
  ...
  'admin.update.text': 'PanelMint {version}이(가) 사용 가능합니다. 현재 {current}을(를) 실행 중입니다.',
  'admin.update.button': 'GitHub에서 보기',
  'admin.update.install': '업데이트 설치',
  'admin.update.confirmTitle': '업데이트를 설치할까요?',
  'admin.update.confirmText': 'PanelMint이 {current}에서 {version}으로 업데이트됩니다. 서버가 이후 자동으로 재시작됩니다.',
  ...
  'admin.update.dockerText':
    'PanelMint 인스턴스가 Docker에서 실행 중입니다. {version}으로 업데이트하려면 서버에서 다음 명령을 실행하세요:',
  'admin.update.nonDockerText':
    '이 PanelMint 인스턴스는 Docker에서 실행되고 있지 않습니다. {version}으로 업데이트하려면 사용했던 설치 또는 업데이트 방법을 다시 실행하세요 — 예를 들어 Proxmox Community Scripts에서는 LXC 콘솔에서 업데이트를 실행하세요:',
