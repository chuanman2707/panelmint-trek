...
const it: NotificationLocale = {
  email: {
    footer: 'Hai ricevuto questa email perché hai le notifiche abilitate in PanelMint.',
    manage: 'Gestisci le preferenze nelle impostazioni',
    madeWith: 'Made with',
    openTrek: 'Apri PanelMint',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} ti ha invitato a fondere i piani vacanza. Apri PanelMint per accettare o rifiutare.`,
    }),
    vacay_share: (p) => ({
      title: 'Calendario Vacay condiviso',
      body: `${p.actor} ha condiviso con te il suo calendario ferie. Apri PanelMint per vederlo.`,
    }),
    collection_invite: (p) => ({
      title: 'Invito a una raccolta',
      body: `${p.actor} ti ha invitato a condividere una raccolta. Apri PanelMint per accettare o rifiutare.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nuova versione PanelMint disponibile',
      body: `PanelMint ${p.version} è ora disponibile. Visita il pannello di amministrazione per aggiornare.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Ciao',
    body: 'Abbiamo ricevuto una richiesta di reimpostazione della password per il tuo account PanelMint. Clicca il pulsante qui sotto per impostare una nuova password.',
