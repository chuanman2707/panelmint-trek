...
const admin: TranslationStrings = {
  ...
  'admin.apiKeys': 'APIキー',
  'admin.apiKeysHint': '場所データの出どころ。PanelMint のインデックスにキーは不要で、下の二つの提供元は任意です。',
  'admin.trekApi.badgeDefault': '推奨の既定ソース',
  'admin.googleCaveat.badge': '推奨しません',
  'admin.googleCaveat.body':
    'PanelMint はオープンソースで、この点で中立ではありません。この規模でレビューとふつうの店舗の写真があるのは Google だけで、それが独占です。この欄は代わりがないから置いてあるのであって、勧めているからではありません。使えば問い合わせはすべて Google に届きます。',
  'admin.trekApi.tagline':
    'PanelMint 自前の場所インデックス。Google のキーなし、上限なし、そして誰にも検索を数えられずに探せます。',
  ...
  'admin.placesProvider.subtitle':
    'PanelMint 自身の索引と OpenStreetMap がすべての検索に答えます。ここでは、それらが何も見つけられなかったときに誰に尋ねるかを選びます。自動はキーがあれば Google を、次に Amap を選びます。',
  'admin.placesProvider.auto': '自動',
  'admin.placesProvider.google': 'Google Places',
  'admin.placesProvider.amap': 'Amap（高德地图）',
  'admin.placesProvider.openstreetmap': 'OpenStreetMap',
  'admin.placesProvider.missingKey': '選択したプロバイダに API キーが設定されていないため、地点検索には PanelMint の索引と OpenStreetMap だけが答えます。',
  ...
  'admin.plugins.requiresTrek': 'PanelMint {version} 以上が必要',
  ...
  'admin.plugins.perm.hook:search-provider': '独自のインデックスから場所の検索に回答（PanelMint 自身の結果と並べて表示）',
  ...
  'admin.plugins.perm.geolocation:read':
    'このプラグインのビューが開いている間、あなたの現在地を取得（PanelMint がこのサイトの位置情報の許可を使って読み取ります。プラグイン専用の許可は求められません）',
  ...
  'admin.plugins.reviewedMeaning':
    '「レビュー済み」とは、PanelMintのメンテナーが各バージョンでこのプラグインをマルウェアの観点から確認したという意味です — 品質や動作の可否を確認したものではありません。プラグインが無害であることを保証するものではありません。',
  ...
  'admin.plugins.security.isolationBody':
    'プラグインは、自身のファイルしか読み取れない厳重に制限された独立プロセスとして実行されます。データベース、ログイン用のシークレット、暗号鍵を読み取ることはできず、他のプログラムを起動することも、どこかにファイルを書き込むこともできません。その画面は隔離されたブラウザフレーム内で動作し、セッションCookieを読み取ったり、周囲のPanelMintページに干渉したりすることはできません。',
  'admin.plugins.security.permsTitle': '権限の意味',
  'admin.plugins.security.permsBody':
    'インストール前に表示される権限は、プラグインの実行中にPanelMintが強制する厳格な制限です — リストにないことは一切実行できません。ただし、これらはプラグインが何をできるかを示すもので、実際に何をするかを示すものではありません。旅行の読み取りとサーバーへの接続を許可されたプラグインは、あなたの旅行をそのサーバーに送信できます。説明文だけでなく、権限と接続先ホストも確認してください。',
  'admin.plugins.security.limitsTitle': '保証できないこと',
  'admin.plugins.security.limitsBody':
    'この分離は強固なソフトウェア上の境界ですが、絶対的なものではありません。プラグインは、あなたが承認した権限のとおりに動作するため、その権限の範囲内では説明とは異なる振る舞いをする場合があり、正当に保持しているデータを宣言済みのホストへ送信することもできます。PanelMintは、プラグインのコードが実際に何をするかを読み取ったり判断したりはしません。',
  ...
  'admin.plugins.security.reviewedBody':
    'レビュー済みのプラグインは、PanelMintのメンテナーが各バージョンで手動でマルウェアの有無を確認したものです — 悪意のあるコードの有無を確認したものであり、正しく動作するかどうかを確認したものではありません。プラグインが無害であることを約束するものではありません。',
  'admin.plugins.security.signedTitle': '「署名済み」の意味',
  'admin.plugins.security.signedBody':
    'PanelMintがインストールのたびに検証するチェックサムは、ファイルがレジストリの保証するものとまったく同一であることを証明します。署名が証明するのは別のことです。そのファイルが作者本人から届いたこと、そして作者だけが持つ鍵で署名されていることです。署名済みのプラグインはその両方を備えています。署名がないプラグインが危険というわけではありません — 保証がひとつ少ないだけであり、現時点ではレジストリのプラグインの多くは署名されていません。',
  ...
  'admin.plugins.sig.missingBody':
    'このプラグインはインストール時には署名されていましたが、新しいバージョンには署名がありません。PanelMintはこの後退を黙って受け入れることはしません。これを上書きすることはできません。',
  ...
  'admin.plugins.sig.confirmOutOfBand':
    'PanelMintには、正当な鍵の更新と乗っ取りを見分けることはできません — ここからは両者はまったく同じに見えます。受け入れる前に、すでに信頼している経路で作者に新しい鍵を確認してください。受け入れると、プラグインが更新され、新しい鍵が記憶されます。',
  ...
  'admin.plugins.security.trustBody':
    'プラグインのインストールは、サードパーティ製アプリのインストールと同じです。信頼できる作者のコードのみを追加し、少しでも疑わしい場合は、まずご自身で内容を確認してください。PanelMintはサードパーティ製プラグインについて一切の責任を負いません。',
  ...
  'admin.plugins.dep.trekIncompatible': 'PanelMint {range} が必要です — このサーバーは {host} です',
  'admin.plugins.dep.trekUnknown': '対応する PanelMint バージョンが宣言されていません',
  ...
  'admin.plugins.rangeBypass.title': 'サポート対象の PanelMint バージョン外',
  'admin.plugins.rangeBypass.noticeTitle': 'サポート対象の PanelMint バージョン外にインストールされました',
  ...
  'admin.plugins.dep.trekBypassed': 'PanelMint 範囲外（{range}）— バージョンチェック無効',
  'admin.plugins.dep.trekBypassedUnknown': 'PanelMint 範囲の宣言なし — バージョンチェック無効',
  ...
  'admin.addons.subtitle': '機能を有効／無効にしてPanelMintをカスタマイズします。',
  ...
  'admin.weather.description':
    'PanelMintは天気データにOpen‑Meteoを使用しています。無料でオープンソース、APIキーは不要です。',
  ...
  'admin.update.text': 'PanelMint {version} が利用可能です。現在は {current} を使用しています。',
  'admin.update.button': 'GitHubで見る',
  'admin.update.install': '更新をインストール',
  'admin.update.confirmTitle': '更新をインストールしますか？',
  'admin.update.confirmText': 'PanelMintを {current} から {version} に更新します。更新後、サーバーは自動的に再起動します。',
  ...
  'admin.update.dockerText':
    'PanelMintはDockerで実行されています。{version} に更新するには、サーバーで次のコマンドを実行してください：',
  'admin.update.nonDockerText':
    'このPanelMintインスタンスはDockerで実行されていません。{version} に更新するには、使用したインストールまたは更新方法をもう一度実行してください。たとえばProxmox Community Scriptsの場合は、LXCコンソールから更新を実行します：',
